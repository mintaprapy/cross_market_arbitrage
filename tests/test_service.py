...
class MonitorServiceTests(unittest.TestCase):
    ...
    def test_snapshot_hides_dashboard_disabled_groups_but_single_group_query_still_reads_them(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            repository = SQLiteRepository(f"{tmp_dir}/monitor.db")
            config = MonitorConfig.model_validate(
                {
            ...
            payload = service.get_snapshot()
            hidden_row = service.query.get_snapshot_row("CF_COTTON_TEST")
